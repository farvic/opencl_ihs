...
void mulmat1_opencl(int32_t* A, int32_t* B, int32_t* C, const uint32_t n, const uint32_t k, const uint32_t m) {
    ...
    Kernel kernel(program, "mulmat1_opencl");
    ...
    // Tempo de início
    delta = high_resolution_clock::now();
    // Enfileirando comando com dimensão global N x M
    queue.enqueueNDRangeKernel(kernel, NullRange, NDRange(N, M));
